	...
	public class AlumnoController {
	    ...
	    /**
	     * Este metodo esta mapeado con la pagina agregaAlumno. El parametro
	     * se envia como /agregaAlumno?matricula=xxx&nombre=xxx
	     * 
	     */
	    @RequestMapping("/agregaAlumno")
	    public String agregaAlumno(@RequestParam(value="matricula", required=true)
	    	int matricula, @RequestParam(value="nombre", required=true) 
	    	String nombre, Model model) {
	            ...
	            //Si se agrego exitosamente mostrar la vista ‘vistaAgregaAlumno’
	
	            if(servicioAlumnos.addAlumno(al)){
	            	model.addAttribute("alumno", al);
	            	return "vistaAgregaAlumno";
	            }
	            //Si no se agrego exitosamente mostrar la vista ‘vistaErrorAgregaAlumno’ (ver Paso 2.3)
	            else{
